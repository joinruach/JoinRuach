...
describe('RateLimiter Service', () => {
  ...
  describe('Window expiration', () => {
    it('should reset after window expires', () => {
      ...
      // Should be blocked
      let result = rateLimiter.check(key, maxAttempts, windowMs);
      expect(result.allowed).toBe(false);
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          result = rateLimiter.check(key, maxAttempts, windowMs);
          expect(result.allowed).toBe(true);
  ...
  describe('Multiple keys', () => {
    it('should track different keys independently', () => {
      ...
      const result1 = rateLimiter.check(key1, maxAttempts, windowMs);
      expect(result1.allowed).toBe(false);

      // key2 should still be allowed
      const result2 = rateLimiter.check(key2, maxAttempts, windowMs);
  ...
  describe('Reset functionality', () => {
    it('should reset rate limit for specific key', () => {
      ...
      // Reset the key
      const wasReset = rateLimiter.reset(key);
    ...
    it('should return false when resetting non-existent key', () => {
      const wasReset = rateLimiter.reset('non:existent:key');
  ...
  describe('Login-specific scenarios', () => {
    it('should enforce IP-based login limits', () => {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        const result = rateLimiter.check(ipKey, maxAttempts, windowMs);
        expect(result.allowed).toBe(true);
      }
    ...
    it('should enforce username-based login limits', () => {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        const result = rateLimiter.check(usernameKey, maxAttempts, windowMs);
        expect(result.allowed).toBe(true);
      }
    ...
    it('should allow successful login to reset username limit', () => {
      const username = 'test@example.com';
      const usernameKey = `login:user:${username}`;
      const maxAttempts = 3;
      const windowMs = 15 * 60 * 1000;
      ...
      rateLimiter.check(usernameKey, maxAttempts, windowMs);
      rateLimiter.check(usernameKey, maxAttempts, windowMs);
  ...
  describe('Media view rate limiting', () => {
    it('should limit views per media item per IP', () => {
      const mediaId = 123;
      ...
      for (let i = 0; i < maxViews; i++) {
        const result = rateLimiter.check(key, maxViews, windowMs);
        expect(result.allowed).toBe(true);
      }
    ...
    it('should track views per media item separately', () => {
      ...
      for (let i = 0; i < maxViews; i++) {
        rateLimiter.check(media1, maxViews, windowMs);
      }

      // media1 should be blocked
      const result1 = rateLimiter.check(media1, maxViews, windowMs);
      expect(result1.allowed).toBe(false);

      // media2 should still be allowed
      const result2 = rateLimiter.check(media2, maxViews, windowMs);
      expect(result2.allowed).toBe(true);
    });
  });
});
