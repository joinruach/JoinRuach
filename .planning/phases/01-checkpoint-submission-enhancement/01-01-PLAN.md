...
1. Create new server action for heartbeat tracking:
   ```typescript
   export async function trackDwellHeartbeat(formData: FormData) {
     ...
     // Store in database (formation-dwell-session table) or in-memory cache (Redis)
     const session = await getOrCreateDwellSession(sectionId, checkpointId, userId);

     // Increment visible seconds by 10
     session.accumulatedSeconds += 10;
     session.lastHeartbeat = new Date();

     await saveDwellSession(session);
...
2. Update submitCheckpoint to validate accumulated dwell time:
   ...
   // Get accumulated dwell time from server session
   const session = await getDwellSession(sectionId, checkpointId, userId);
   ...
   await clearDwellSession(sectionId, checkpointId, userId);
   ```

**CLIENT-SIDE (SectionView.tsx):**

1. Add state for elapsed time and tab visibility (use timestamps, not increments):
   ```typescript
   ...
   useEffect(() => {
     const handleVisibilityChange = () => {
       ...
       if (isVisible) {
         ...
         // Tab became hidden - accumulate time since last visible
         if (lastVisibleAt !== null) {
           const elapsed = Math.floor((performance.now() - lastVisibleAt) / 1000);
...
3. Update display time every second (for UX only - server time is truth):
   ```typescript
   ...
   useEffect(() => {
     const interval = setInterval(() => {
       if (isTabVisible && lastVisibleAt !== null) {
         const currentElapsed = Math.floor((performance.now() - lastVisibleAt) / 1000);
   ...
   }, [isTabVisible, lastVisibleAt, accumulatedSeconds]);
   ```

4. Send heartbeat ping every 10 seconds (only while visible):
   ```typescript
   useEffect(() => {
     ...
     const heartbeatInterval = setInterval(async () => {
       if (isTabVisible) {
         ...
         await trackDwellHeartbeat(formData);
       }
     }, 10000); // 10 seconds

     return () => clearInterval(heartbeatInterval);
   }, [isTabVisible, section.id, checkpoint.id, userId]);
   ```
...
6. Add dwell time progress display:
   ```typescript
...
7. Requirements checklist + submit button (same as before):
   ```typescript
...
**STORAGE OPTIONS for dwell sessions:**
- **Option A (simple):** Redis with key `dwell:${userId}:${checkpointId}` (TTL 1 hour)
- **Option B (persistent):** Database table `formation_dwell_sessions` (userId, checkpointId, accumulatedSeconds, lastHeartbeat)

Use Option A (Redis) for Phase 1 - simpler, ephemeral, sufficient for formation gate.

**WHY heartbeat:** Server cannot be tricked - even if client modifies timer display or sends fake submission, server only accepts if it received heartbeat pings. Client can skip heartbeats (gaming detection: no pings = reject), but cannot fake accumulated time.
  </action>
  <verify>
CLIENT-SIDE UX:
