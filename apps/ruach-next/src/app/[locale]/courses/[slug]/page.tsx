...
import MiniCourseLanding from "@/components/ruach/MiniCourseLanding";
...
import { AccessLevel, ACCESS_LEVEL_RANK, normalizeAccessLevel } from "@ruach/utils";
...
export default async function CourseDetail({ params }: { params: Promise<{ slug: string }> }){
  ...
  const lessons: ParsedLesson[] = lessonsRaw
    ...
    .map((attrs) => {
      ...
      const rawAccess = normalizeAccessLevel((attrs.requiredAccessLevel as string | undefined) ?? null);
      return {
        slug: slugValue,
  ...
  const membershipAccessLevel = normalizeAccessLevel(membership?.accessLevel ?? null);
  ...
  const miniLanding = a.landingConfig;
  const isMiniCourse = Boolean(miniLanding?.hero?.title);
  const detoxSlug = a.ctaDetoxCourse?.data?.attributes?.slug;
  const startLessonSlug = firstLesson?.slug ?? resumeLesson?.slug ?? slug;

  return (
    <div className="space-y-12">
      <SEOHead jsonLd={courseSchema} />
      {isMiniCourse && miniLanding ? (
        <MiniCourseLanding
          slug={slug}
          landingConfig={miniLanding}
          firstLessonSlug={startLessonSlug}
          detoxSlug={detoxSlug ?? undefined}
        />
      ) : (
        <>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {lesson.requiredAccessLevel !== "public" ? (
